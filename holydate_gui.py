...
class MainWindow(QtGui.QMainWindow):
    ...
    def __init__(self):
        ...
        self.setWindowTitle(QtCore.QString(
            u'Древлеправославный календарь 0.1a1'))
        self.setWindowIcon(QtGui.QIcon(LOGO_SVG))

        geometry = QtGui.QDesktopWidget().screenGeometry()
        height = (geometry.height() / 100) * 90
        width = (geometry.width() / 100) * 43
        print height, width
...
class MainWidget(QtGui.QWidget):
    def __init__(self, parent=MainWindow):
        ...
        self.rbNewStyle = QtGui.QRadioButton(
            QtCore.QString(u'По но&вому стилю')
        )
        self.rbNewStyle.setChecked(True)
        self.rbOldStyle = QtGui.QRadioButton(
            QtCore.QString(u'По ста&рому стилю')
        )
        ...
        self.gregorianDateEditForm.setDisplayFormat(
            QtCore.QString('dd MMMM yyyy')
        )
        ...
        # Embed custom fonts.
        pt_serif_custom = QtGui.QFontDatabase.addApplicationFont(os.path.join(
            DIR, 'fonts/PTSerif-Custom.ttf')
        )
        pt_serif_caption = QtGui.QFontDatabase.addApplicationFont(os.path.join(
            DIR, 'fonts/PTZ55F.ttf')
        )
        pt_serif_bold = QtGui.QFontDatabase.addApplicationFont(
            os.path.join(DIR, 'fonts/PTF75F.ttf')
        )
        ...
        self.gregorianCalendarWidget.selectionChanged.connect(
            self.setSelectedDateJulian
        )
        self.julianCalendarWidget.selectionChanged.connect(
            self.setSelectedDateGregorian
        )
        # Change current page between widget.
        # Fixme: this slots called undesirable recursion.
        self.gregorianCalendarWidget.currentPageChanged.connect(
            self.changeJulianCurrentPage
        )
        self.julianCalendarWidget.currentPageChanged.connect(
            self.changeGregorianCurrentPage
        )
        ...
        #Date edit form.
        self.gregorianDateEditForm.dateChanged.connect(
            self.setGregorianCalendarDate
        )
        self.julianDateEditForm.dateChanged.connect(
            self.setJulianCalendarDate
        )
        self.gregorianCalendarWidget.clicked.connect(
            self.setGregorianDateEditForm
        )
        self.julianCalendarWidget.clicked.connect(
            self.setJulianDateEditForm
        )
        #Calendar signal.
        self.gregorianCalendarWidget.currentPageChanged.connect(
            self.setGregorianBrushedCell
    ...
    def setGregorianCalendar(self):
        ...
        self.gregorianDateEditForm.setDate(
            self.gregorianCalendarWidget.selectedDate()
        )
    ...
    def setJulianCalendar(self):
        ...
        self.julianDateEditForm.setDate(
            self.julianCalendarWidget.selectedDate()
        )
    ...
    def setDateToday(self):
        ...
        if self.calendar_system == 'gregorian':
            self.gregorianCalendarWidget.setSelectedDate(
    ...
    def setGregorianBrushedCell(self):
        ...
        for month in range(1, 13):
            ...
            for day in range(1, days_in_month):
                ...
                fast_and_feast.setdefault(month, {}).update(
                    {day: cal.getFeastAndFastStatus()})
            ...
            for month in range(1, 13):
                ...
                for day in range(1, days_in_month):
                    if fast_and_feast[month][day] is 3:
                        ...
                        f.setBackground(QtGui.QBrush(QtGui.QColor(WHITE)))
                    self.gregorianCalendarWidget.setDateTextFormat(
                        QtCore.QDate(year, month, day), f)
        except KeyError:
            pass
            ...
            for month in range(1, 13):
                ...
                for day in range(1, days_in_month):
                    if fast_and_feast[month][day] is 3:
                        ...
                        f.setForeground(QtGui.QBrush(QtGui.QColor(BLACK)))
                        f.setBackground(QtGui.QBrush(QtGui.QColor(WHITE)))
                    self.julianCalendarWidget.setDateTextFormat(
                        QtCore.QDate(year, month, day), f)
        except KeyError:
            pass

    def calendar_constructor(self, day, month, year):
        calendar = AncientCalendar(
            day, month, year, calendar=self.calendar_system)
        ...
        weekdayname = calendar.getWeekdayname().format(
            red="<span class='red'>",
        ...
        saints = calendar.getSaint().format(
            red="<span class='red'>",
            bold="<span class='bold'>",
            end="</span>",
            tw="<span class='twelve'>ӱ</span>",
        ...
        out = """
              <html>
                  <head>
                     <link rel="stylesheet"
                           media="all" type="text/css" href="{css}" />
                  </head>
                <body>
                  <div>
                    <p>
    ...
    def output(self):
        if self.calendar_system == 'gregorian':
            ...
            _gr_month = int(
                self.gregorianCalendarWidget.selectedDate().month())
    ...
    def searchFormEnter(self):
        ...
        result = QtCore.QString(result.format(
...
class HolydateGui(AncientCalendar):
    def getFeastAndFastStatus(self):
        ...
        #Feast.
            ...
            index = 1
        elif saint in [2, 3, 4, 5, 6, 7] and self.fast in \
                [6, 7, 8, 9, 10, 11, 12, 13]:
            index = 1
        #Feast and fast.
...
class AboutDialog(QtGui.QMessageBox):
    def __init__(self):
        ...
        text = u"""
            <h3>holydate-gui</h3>
            <h4>Версия 0.1а1</h4>
            <p><a href='http://www.vechnoe.info/holydate'>
                http://www.vechnoe.info/holydate</a></p>
            ...
            <p><b>holydate-gui</b> — GUI интерфейс к python-билиотеке
                <b>holydate:</b>
            <a href='https://pypi.python.org/pypi/holydate'>
                https://pypi.python.org/pypi/holydate</a>

            <h4>Лицензия</h4>
            <p>Copyright &copy; 2014 Максим Чернятевич.</p>
            <p><b>holydate-gui</b>
                распространяется на условиях лицензии GNU GPLv3.
            <a href='https://www.gnu.org/copyleft/gpl.html'>
                https://www.gnu.org/copyleft/gpl.html</a>
            </p>

            <h4>Сообщения об ошибках и пожелания отправляйте на почту:</h4>
            <p><a href='mailto:vechnoe.info@gmail.com'>
                vechnoe.info@gmail.com</a></p>
            """
