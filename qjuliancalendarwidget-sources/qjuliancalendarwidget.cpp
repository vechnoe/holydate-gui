...
#ifndef QT_NO_JULIANCALENDARWIDGET
...
class QJulianCalendarDateSectionValidator
{
public:
    ...
    QJulianCalendarDateSectionValidator() {}
    virtual ~QJulianCalendarDateSectionValidator() {}
...
class QJulianCalendarMonthValidator : public QJulianCalendarDateSectionValidator
{

public:
    QJulianCalendarMonthValidator();
...
class QJulianCalendarYearValidator : public QJulianCalendarDateSectionValidator
{

public:
    QJulianCalendarYearValidator();
    virtual Section handleKey(int key);
    virtual QDate applyToDate(const QDate &date) const;
    virtual void setDate(const QDate &date);
    virtual QString text() const;
...
class QJulianCalendarDateValidator
{
public:
    QJulianCalendarDateValidator();
    ~QJulianCalendarDateValidator();
...
private:
    ...
    struct SectionToken {
        SectionToken(QJulianCalendarDateSectionValidator *val, int rep) : validator(val), repeat(rep) {}
        QJulianCalendarDateSectionValidator *validator;
        int repeat;
    };
    ...
    QJulianCalendarDateSectionValidator *m_yearValidator;
    QJulianCalendarDateSectionValidator *m_monthValidator;
    QJulianCalendarDateSectionValidator *m_dayValidator;
    ...
    QJulianCalendarDateSectionValidator::Section m_lastSectionMove;
};

class QJulianCalendarView : public QTableView
{
    Q_OBJECT
public:
    QJulianCalendarView(QWidget *parent = 0);
...
class QJulianCalendarModel : public QAbstractTableModel
{
    Q_OBJECT
public:
    QJulianCalendarModel(QObject *parent = 0);
    ...
    void setHorizontalHeaderFormat(QJulianCalendarWidget::HorizontalHeaderFormat format);
    ...
    void setView(QJulianCalendarView *view)
        { m_view = view; }
    ...
    int m_firstRow;
    QDate m_date;
    QDate m_gregorian_date;
    QDate m_minimumDate;
    QDate m_maximumDate;
    int m_shownYear;
    int m_shownMonth;
    Qt::DayOfWeek m_firstDay;
    QJulianCalendarWidget::HorizontalHeaderFormat m_horizontalHeaderFormat;
    ...
    QJulianCalendarView *m_view;
};

class QJulianCalendarDelegate : public QItemDelegate
{
    Q_OBJECT
public:
    QJulianCalendarDelegate(QJulianCalendarWidgetPrivate *w, QObject *parent = 0)
...
private:
    QJulianCalendarWidgetPrivate *calendarWidgetPrivate;
...
class QJulianCalendarWidgetPrivate : public QWidgetPrivate
{
    Q_DECLARE_PUBLIC(QJulianCalendarWidget)
public:
    QJulianCalendarWidgetPrivate();
    ...
    QJulianCalendarModel *m_model;
    QJulianCalendarView *m_view;
    QJulianCalendarDelegate *m_delegate;
...
QString QJulianCalendarDateSectionValidator::highlightString(const QString &str, int pos) const
{
    if (pos == 0)
...
class QJulianCalendarDayValidator : public QJulianCalendarDateSectionValidator
{
 
public:
    QJulianCalendarDayValidator();
    virtual Section handleKey(int key);
    virtual QDate applyToDate(const QDate &date) const;
    virtual void setDate(const QDate &date);
    virtual QString text() const;
...
QJulianCalendarDayValidator::QJulianCalendarDayValidator()
    : QJulianCalendarDateSectionValidator(), m_pos(0), m_day(1), m_oldDay(1)
{
}
 
QJulianCalendarDateSectionValidator::Section QJulianCalendarDayValidator::handleKey(int key)
{
    if (key == Qt::Key_Right || key == Qt::Key_Left) {
        m_pos = 0;
        return QJulianCalendarDateSectionValidator::ThisSection;
        ...
        if (m_pos == 0)
            return QJulianCalendarDateSectionValidator::PrevSection;
        return QJulianCalendarDateSectionValidator::ThisSection;
    }
    ...
    if (m_pos > 1) {
        m_pos = 0;
        return QJulianCalendarDateSectionValidator::NextSection;
    }
    return QJulianCalendarDateSectionValidator::ThisSection;
}
 
QDate QJulianCalendarDayValidator::applyToDate(const QDate &date) const
{
    int day = m_day;
    if (day < 1)
        day = 1;
...
void QJulianCalendarDayValidator::setDate(const QDate &date)
{
    m_day = m_oldDay = date.day();
    m_pos = 0;
}
 
QString QJulianCalendarDayValidator::text() const
{
    QString str;
    if (m_day / 10 == 0)
        str += QLatin1Char('0');
...
QString QJulianCalendarDayValidator::text(const QDate &date, int repeat) const
{
    if (repeat <= 1) {
...
QJulianCalendarMonthValidator::QJulianCalendarMonthValidator()
    : QJulianCalendarDateSectionValidator(), m_pos(0), m_month(1), m_oldMonth(1)
{
}
 
QJulianCalendarDateSectionValidator::Section QJulianCalendarMonthValidator::handleKey(int key)
{
    if (key == Qt::Key_Right || key == Qt::Key_Left) {
        m_pos = 0;
        return QJulianCalendarDateSectionValidator::ThisSection;
...
QDate QJulianCalendarMonthValidator::applyToDate(const QDate &date) const
{
    int month = m_month;
    if (month < 1)
        month = 1;
...
void QJulianCalendarMonthValidator::setDate(const QDate &date)
{
    m_month = m_oldMonth = date.month();
    m_pos = 0;
}
 
QString QJulianCalendarMonthValidator::text() const
{
    QString str;
    if (m_month / 10 == 0)
        str += QLatin1Char('0');
...
QString QJulianCalendarMonthValidator::text(const QDate &date, int repeat) const
{
    if (repeat <= 1) {
...
QJulianCalendarYearValidator::QJulianCalendarYearValidator()
    : QJulianCalendarDateSectionValidator(), m_pos(0), m_year(2000), m_oldYear(2000)
{
}
 
int QJulianCalendarYearValidator::pow10(int n)
{
    int power = 1;
    for (int i = 0; i < n; i++)
        power *= 10;
...
QJulianCalendarDateSectionValidator::Section QJulianCalendarYearValidator::handleKey(int key)
{
    if (key == Qt::Key_Right || key == Qt::Key_Left) {
        m_pos = 0;
        return QJulianCalendarDateSectionValidator::ThisSection;
...
QDate QJulianCalendarYearValidator::applyToDate(const QDate &date) const
{
    int year = m_year;
    if (year < 1)
        year = 1;
...
void QJulianCalendarYearValidator::setDate(const QDate &date)
{
    m_year = m_oldYear = date.year();
    m_pos = 0;
}
 
QString QJulianCalendarYearValidator::text() const
{
    QString str;
    int pow = 10;
    for (int i = 0; i < 3; i++) {
...
QString QJulianCalendarYearValidator::text(const QDate &date, int repeat) const
{
    if (repeat < 4) {
        QString str;
...
QJulianCalendarDateValidator::QJulianCalendarDateValidator()
    : m_currentToken(0), m_lastSectionMove(QJulianCalendarDateSectionValidator::ThisSection)
{
    m_initialDate = m_currentDate = QDate::currentDate();
    m_yearValidator = new QJulianCalendarYearValidator();
    m_monthValidator = new QJulianCalendarMonthValidator();
    m_dayValidator = new QJulianCalendarDayValidator();
}
 
void QJulianCalendarDateValidator::setLocale(const QLocale &locale)
{
...
// from qdatetime.cpp
int QJulianCalendarDateValidator::countRepeat(const QString &str, int index) const
{
...
void QJulianCalendarDateValidator::setInitialDate(const QDate &date)
{
    ...
    m_initialDate = date;
    m_currentDate = date;
    m_lastSectionMove = QJulianCalendarDateSectionValidator::ThisSection;
}
 
QString QJulianCalendarDateValidator::currentText() const
{
    QString str;
    ...
    while (itSep.hasNext()) {
        ...
        if (itTok.hasNext()) {
            ...
            QJulianCalendarDateSectionValidator *validator = token->validator;
...
void QJulianCalendarDateValidator::clear()
{
...
void QJulianCalendarDateValidator::setFormat(const QString &format)
{
    clear();
...
void QJulianCalendarDateValidator::applyToDate()
{
...
void QJulianCalendarDateValidator::toNextToken()
{
...
void QJulianCalendarDateValidator::toPreviousToken()
{
    const int idx = m_tokens.indexOf(m_currentToken);
    if (idx == -1)
        return;
...
void QJulianCalendarDateValidator::handleKeyEvent(QKeyEvent *keyEvent)
{
    if (!m_currentToken)
        return;
 
    int key = keyEvent->key();
    if (m_lastSectionMove == QJulianCalendarDateSectionValidator::NextSection) {
    ...
    if (m_lastSectionMove == QJulianCalendarDateSectionValidator::NextSection)
        toNextToken();
    else if (m_lastSectionMove == QJulianCalendarDateSectionValidator::PrevSection)
        toPreviousToken();
}
...
void QCalendarTextNavigator::createDateLabel()
{
    ...
    m_dateValidator = new QJulianCalendarDateValidator();
...
class QJulianCalendarView;
 
 
QJulianCalendarModel::QJulianCalendarModel(QObject *parent)
    : QAbstractTableModel(parent)
{
    m_gregorian_date = QDate::currentDate();
    QDate ReturnDate;
    ReturnDate = QDate::fromJulianDay(m_gregorian_date.toJulianDay()-13);
    m_date = ReturnDate;
    ...
    m_horizontalHeaderFormat = QJulianCalendarWidget::ShortDayNames;
...
Qt::DayOfWeek QJulianCalendarModel::dayOfWeekForColumn(int column) const
{
...
int QJulianCalendarModel::columnForDayOfWeek(Qt::DayOfWeek day) const
{
    if (day < 1 || day > 7)
        return -1;
...
*/
QDate QJulianCalendarModel::referenceDate() const
{
    int refDay = 1;
    while (refDay <= 31) {
...
int QJulianCalendarModel::columnForFirstOfMonth(const QDate &date) const
{
    return (columnForDayOfWeek(static_cast<Qt::DayOfWeek>(date.dayOfWeek())) - (date.day() % 7) + 7) % 7;//8
}
 
QDate QJulianCalendarModel::dateForCell(int row, int column) const
{
...
void QJulianCalendarModel::cellForDate(const QDate &date, int *row, int *column) const
{
    if (!row && !column)
        return;
...
QString QJulianCalendarModel::dayName(Qt::DayOfWeek day) const
{
    switch (m_horizontalHeaderFormat) {
        case QJulianCalendarWidget::SingleLetterDayNames: {
        ...
        case QJulianCalendarWidget::ShortDayNames:
            return m_view->locale().dayName(day, QLocale::ShortFormat);
        case QJulianCalendarWidget::LongDayNames:
...
QTextCharFormat QJulianCalendarModel::formatForCell(int row, int col) const
{
    QPalette pal;
    ...
    bool header = (m_weekNumbersShown && col == HeaderColumn)
                  || (m_horizontalHeaderFormat != QJulianCalendarWidget::NoHorizontalHeader && row == HeaderRow);
...
QVariant QJulianCalendarModel::data(const QModelIndex &index, int role) const
{
    ...
    if(role == Qt::DisplayRole) {
        ...
        if (m_horizontalHeaderFormat != QJulianCalendarWidget::NoHorizontalHeader && row == HeaderRow
...
Qt::ItemFlags QJulianCalendarModel::flags(const QModelIndex &index) const
{
...
void QJulianCalendarModel::setDate(const QDate &d)
{
    m_date = d;
...
void QJulianCalendarModel::showMonth(int year, int month)
{
...
void QJulianCalendarModel::setMinimumDate(const QDate &d)
{
...
void QJulianCalendarModel::setMaximumDate(const QDate &d)
{
...
void QJulianCalendarModel::setRange(const QDate &min, const QDate &max)
{
    m_minimumDate = min;
    m_maximumDate = max;
    if (m_minimumDate > m_maximumDate)
...
void QJulianCalendarModel::internalUpdate()
{
...
void QJulianCalendarModel::setHorizontalHeaderFormat(QJulianCalendarWidget::HorizontalHeaderFormat format)
{
    ...
    if (oldFormat == QJulianCalendarWidget::NoHorizontalHeader) {
        m_firstRow = 1;
        insertRow(0);
    } else if (m_horizontalHeaderFormat == QJulianCalendarWidget::NoHorizontalHeader) {
        m_firstRow = 0;
        removeRow(0);
    }
    internalUpdate();
...
void QJulianCalendarModel::setFirstColumnDay(Qt::DayOfWeek dayOfWeek)
{
...
Qt::DayOfWeek QJulianCalendarModel::firstColumnDay() const
{
    return m_firstDay;
}
 
bool QJulianCalendarModel::weekNumbersShown() const
{
    return m_weekNumbersShown;
}
 
void QJulianCalendarModel::setWeekNumbersShown(bool show)
{
...
QJulianCalendarView::QJulianCalendarView(QWidget *parent)
    : QTableView(parent),
    readOnly(false),
...
QModelIndex QJulianCalendarView::moveCursor(CursorAction cursorAction, Qt::KeyboardModifiers modifiers)
{
    QJulianCalendarModel *calendarModel = qobject_cast<QJulianCalendarModel *>(model());
    if (!calendarModel)
    ...
    QDate currentDate = static_cast<QJulianCalendarModel*>(model())->dateForCell(index.row(), index.column());
    switch (cursorAction) {
...
void QJulianCalendarView::keyPressEvent(QKeyEvent *event)
{
#ifdef QT_KEYPAD_NAVIGATION
    if (event->key() == Qt::Key_Select) {
        ...
        if (QApplication::keypadNavigationEnabled() && hasEditFocus()) {
            if (qobject_cast<QJulianCalendarModel *>(model())) {
...
#ifndef QT_NO_WHEELEVENT
void QJulianCalendarView::wheelEvent(QWheelEvent *event)
{
...
bool QJulianCalendarView::event(QEvent *event)
{
#ifdef QT_KEYPAD_NAVIGATION
    if (event->type() == QEvent::FocusIn) {
        if (QJulianCalendarModel *calendarModel = qobject_cast<QJulianCalendarModel *>(model())) {
...
QDate QJulianCalendarView::handleMouseEvent(QMouseEvent *event)
{
    QJulianCalendarModel *calendarModel = qobject_cast<QJulianCalendarModel *>(model());
    if (!calendarModel)
        return QDate();
...
void QJulianCalendarView::mouseDoubleClickEvent(QMouseEvent *event)
{
    QJulianCalendarModel *calendarModel = qobject_cast<QJulianCalendarModel *>(model());
    if (!calendarModel) {
...
void QJulianCalendarView::mousePressEvent(QMouseEvent *event)
{
    QJulianCalendarModel *calendarModel = qobject_cast<QJulianCalendarModel *>(model());
    if (!calendarModel) {
    ...
    if (date.isValid()) {
        ...
        int row = -1, col = -1;
        static_cast<QJulianCalendarModel *>(model())->cellForDate(date, &row, &col);
        if (row != -1 && col != -1) {
...
void QJulianCalendarView::mouseMoveEvent(QMouseEvent *event)
{
    QJulianCalendarModel *calendarModel = qobject_cast<QJulianCalendarModel *>(model());
    if (!calendarModel) {
...
void QJulianCalendarView::mouseReleaseEvent(QMouseEvent *event)
{
    QJulianCalendarModel *calendarModel = qobject_cast<QJulianCalendarModel *>(model());
    if (!calendarModel) {
...
void QJulianCalendarDelegate::paint(QPainter *painter, const QStyleOptionViewItem &option,
            const QModelIndex &index) const
{
...
void QJulianCalendarDelegate::paintCell(QPainter *painter, const QRect &rect, const QDate &date) const
{
...
QJulianCalendarWidgetPrivate::QJulianCalendarWidgetPrivate()
    : QWidgetPrivate()
{
    m_model = 0;
    m_view = 0;
...
void QJulianCalendarWidgetPrivate::setNavigatorEnabled(bool enable)
{
    Q_Q(QJulianCalendarWidget);
...
void QJulianCalendarWidgetPrivate::createNavigationBar(QWidget *widget)
{
    Q_Q(QJulianCalendarWidget);
...
void QJulianCalendarWidgetPrivate::updateButtonIcons()
{
    Q_Q(QJulianCalendarWidget);
...
void QJulianCalendarWidgetPrivate::updateMonthMenu()
{
    int beg = 1, end = 12;
...
void QJulianCalendarWidgetPrivate::updateMonthMenuNames()
{
    Q_Q(QJulianCalendarWidget);
...
void QJulianCalendarWidgetPrivate::updateCurrentPage(const QDate &date)
{
    Q_Q(QJulianCalendarWidget);
...
void QJulianCalendarWidgetPrivate::_q_monthChanged(QAction *act)
{
...
QDate QJulianCalendarWidgetPrivate::getCurrentDate()
{
    QModelIndex index = m_view->currentIndex();
    return m_model->dateForCell(index.row(), index.column());
}
 
void QJulianCalendarWidgetPrivate::_q_prevMonthClicked()
{
    QDate currentDate = getCurrentDate().addMonths(-1);
    updateCurrentPage(currentDate);
}
 
void QJulianCalendarWidgetPrivate::_q_nextMonthClicked()
{
    QDate currentDate = getCurrentDate().addMonths(1);
    updateCurrentPage(currentDate);
}
 
void QJulianCalendarWidgetPrivate::_q_yearEditingFinished()
{
    Q_Q(QJulianCalendarWidget);
...
void QJulianCalendarWidgetPrivate::_q_yearClicked()
{
    Q_Q(QJulianCalendarWidget);
...
void QJulianCalendarWidgetPrivate::showMonth(int year, int month)
{
...
void QJulianCalendarWidgetPrivate::updateNavigationBar()
{
    Q_Q(QJulianCalendarWidget);
...
void QJulianCalendarWidgetPrivate::update()
{
...
void QJulianCalendarWidgetPrivate::paintCell(QPainter *painter, const QRect &rect, const QDate &date) const
{
    Q_Q(const QJulianCalendarWidget);
    q->paintCell(painter, rect, date);
}
 
void QJulianCalendarWidgetPrivate::_q_slotShowDate(const QDate &date)
{
    updateCurrentPage(date);
}
 
void QJulianCalendarWidgetPrivate::_q_slotChangeDate(const QDate &date)
{
    _q_slotChangeDate(date, true);
}
 
void QJulianCalendarWidgetPrivate::_q_slotChangeDate(const QDate &date, bool changeMonth)
{
...
void QJulianCalendarWidgetPrivate::_q_editingFinished()
{
    Q_Q(QJulianCalendarWidget);
...
/*!
    \class QJulianCalendarWidget
    \brief The QJulianCalendarWidget class provides a monthly based
...
/*!
    ...
    \sa setCurrentPage()
*/
QJulianCalendarWidget::QJulianCalendarWidget(QWidget *parent)
    : QWidget(*new QJulianCalendarWidgetPrivate, parent, 0)
{
    Q_D(QJulianCalendarWidget);
    ...
    d->m_delegate = new QJulianCalendarDelegate(d, this);
...
*/
QJulianCalendarWidget::~QJulianCalendarWidget()
{
}
...
/*!
   \reimp
*/
QSize QJulianCalendarWidget::sizeHint() const
{
...
/*!
   \reimp
*/
QSize QJulianCalendarWidget::minimumSizeHint() const
{
    Q_D(const QJulianCalendarWidget);
    ...
    if (horizontalHeaderFormat() == QJulianCalendarWidget::NoHorizontalHeader) {
        rows = 6;
    } else {
        for (int i = 1; i <= 7; i++) {
    ...
    if (verticalHeaderFormat() == QJulianCalendarWidget::NoVerticalHeader) {
        cols = 7;
    } else {
        for (int i = 1; i <= 6; i++) {
...
void QJulianCalendarWidget::paintCell(QPainter *painter, const QRect &rect, const QDate &date) const
{
    Q_D(const QJulianCalendarWidget);
...
QDate QJulianCalendarWidget::selectedDate() const
{
    Q_D(const QJulianCalendarWidget);
    return d->m_model->m_date;
}
 
void QJulianCalendarWidget::setSelectedDate(const QDate &date)
{
    Q_D(QJulianCalendarWidget);
...
int QJulianCalendarWidget::yearShown() const
{
    Q_D(const QJulianCalendarWidget);
...
int QJulianCalendarWidget::monthShown() const
{
    Q_D(const QJulianCalendarWidget);
...
void QJulianCalendarWidget::setCurrentPage(int year, int month)
{
    Q_D(QJulianCalendarWidget);
...
void QJulianCalendarWidget::showNextMonth()
{
    int year = yearShown();
...
void QJulianCalendarWidget::showPreviousMonth()
{
    int year = yearShown();
    int month = monthShown();
    if (month == 1) {
...
void QJulianCalendarWidget::showNextYear()
{
    int year = yearShown();
    int month = monthShown();
    ++year;
...
void QJulianCalendarWidget::showPreviousYear()
{
    int year = yearShown();
    int month = monthShown();
    --year;
...
*/
void QJulianCalendarWidget::showSelectedDate()
{
...
/*!
    ...
    \sa selectedDate(), setCurrentPage()
*/
void QJulianCalendarWidget::showToday()
{
...
QDate QJulianCalendarWidget::minimumDate() const
{
    Q_D(const QJulianCalendarWidget);
    return d->m_model->m_minimumDate;
}
 
void QJulianCalendarWidget::setMinimumDate(const QDate &date)
{
    Q_D(QJulianCalendarWidget);
...
QDate QJulianCalendarWidget::maximumDate() const
{
    Q_D(const QJulianCalendarWidget);
    return d->m_model->m_maximumDate;
}
 
void QJulianCalendarWidget::setMaximumDate(const QDate &date)
{
    Q_D(QJulianCalendarWidget);
...
void QJulianCalendarWidget::setDateRange(const QDate &min, const QDate &max)
{
    Q_D(QJulianCalendarWidget);
...
void QJulianCalendarWidget::setHorizontalHeaderFormat(QJulianCalendarWidget::HorizontalHeaderFormat format)
{
    Q_D(QJulianCalendarWidget);
...
QJulianCalendarWidget::HorizontalHeaderFormat QJulianCalendarWidget::horizontalHeaderFormat() const
{
    Q_D(const QJulianCalendarWidget);
...
QJulianCalendarWidget::VerticalHeaderFormat QJulianCalendarWidget::verticalHeaderFormat() const
{
    Q_D(const QJulianCalendarWidget);
    bool shown = d->m_model->weekNumbersShown();
    if (shown)
        return QJulianCalendarWidget::ISOWeekNumbers;
    return QJulianCalendarWidget::NoVerticalHeader;
}
 
void QJulianCalendarWidget::setVerticalHeaderFormat(QJulianCalendarWidget::VerticalHeaderFormat format)
{
    Q_D(QJulianCalendarWidget);
    bool show = false;
    if (format == QJulianCalendarWidget::ISOWeekNumbers)
        show = true;
...
bool QJulianCalendarWidget::isGridVisible() const
{
    Q_D(const QJulianCalendarWidget);
    return d->m_view->showGrid();
}
 
void QJulianCalendarWidget::setGridVisible(bool show)
{
    Q_D(QJulianCalendarWidget);
...
QJulianCalendarWidget::SelectionMode QJulianCalendarWidget::selectionMode() const
{
    Q_D(const QJulianCalendarWidget);
    return d->m_view->readOnly ? QJulianCalendarWidget::NoSelection : QJulianCalendarWidget::SingleSelection;
}
 
void QJulianCalendarWidget::setSelectionMode(SelectionMode mode)
{
    Q_D(QJulianCalendarWidget);
    d->m_view->readOnly = (mode == QJulianCalendarWidget::NoSelection);
    d->setNavigatorEnabled(isDateEditEnabled() && (selectionMode() != QJulianCalendarWidget::NoSelection));
    d->update();
}
...
void QJulianCalendarWidget::setFirstDayOfWeek(Qt::DayOfWeek dayOfWeek)
{
    Q_D(QJulianCalendarWidget);
...
Qt::DayOfWeek QJulianCalendarWidget::firstDayOfWeek() const
{
    Q_D(const QJulianCalendarWidget);
...
*/
QTextCharFormat QJulianCalendarWidget::headerTextFormat() const
{
    Q_D(const QJulianCalendarWidget);
...
/*!
    ...
    the header's format.
*/
void QJulianCalendarWidget::setHeaderTextFormat(const QTextCharFormat &format)
{
    Q_D(QJulianCalendarWidget);
...
/*!
    ...
    \sa headerTextFormat()
*/
QTextCharFormat QJulianCalendarWidget::weekdayTextFormat(Qt::DayOfWeek dayOfWeek) const
{
    Q_D(const QJulianCalendarWidget);
...
*/
void QJulianCalendarWidget::setWeekdayTextFormat(Qt::DayOfWeek dayOfWeek, const QTextCharFormat &format)
{
    Q_D(QJulianCalendarWidget);
...
*/
QMap<QDate, QTextCharFormat> QJulianCalendarWidget::dateTextFormat() const
{
    Q_D(const QJulianCalendarWidget);
...
*/
QTextCharFormat QJulianCalendarWidget::dateTextFormat(const QDate &date) const
{
    Q_D(const QJulianCalendarWidget);
...
*/
void QJulianCalendarWidget::setDateTextFormat(const QDate &date, const QTextCharFormat &format)
{
    Q_D(QJulianCalendarWidget);
    if (date.isNull())
...
*/
bool QJulianCalendarWidget::isDateEditEnabled() const
{
    Q_D(const QJulianCalendarWidget);
    return d->m_dateEditEnabled;
}
 
void QJulianCalendarWidget::setDateEditEnabled(bool enable)
{
    Q_D(QJulianCalendarWidget);
    ...
    d->setNavigatorEnabled(enable && (selectionMode() != QJulianCalendarWidget::NoSelection));
}
...
*/
int QJulianCalendarWidget::dateEditAcceptDelay() const
{
    Q_D(const QJulianCalendarWidget);
    return d->m_navigator->dateEditAcceptDelay();
}
 
void QJulianCalendarWidget::setDateEditAcceptDelay(int delay)
{
    Q_D(QJulianCalendarWidget);
...
*/
void QJulianCalendarWidget::updateCell(const QDate &date)
{
    if (!date.isValid()) {
...
/*!
    ...
    \sa updateCell()
*/
void QJulianCalendarWidget::updateCells()
{
    Q_D(QJulianCalendarWidget);
    if (isVisible())
        d->m_view->viewport()->update();
...
*/
bool QJulianCalendarWidget::isHeaderVisible() const
{
    Q_D(const QJulianCalendarWidget);
...
*/
void QJulianCalendarWidget::setHeaderVisible(bool visible)
{
...
void QJulianCalendarWidget::setNavigationBarVisible(bool visible)
{
    Q_D(QJulianCalendarWidget);
...
/*!
  \reimp
*/
bool QJulianCalendarWidget::event(QEvent *event)
{
    Q_D(QJulianCalendarWidget);
    switch (event->type()) {
...
/*!
  \reimp
*/
bool QJulianCalendarWidget::eventFilter(QObject *watched, QEvent *event)
{
    Q_D(QJulianCalendarWidget);
...
/*!
  \reimp
*/
void QJulianCalendarWidget::mousePressEvent(QMouseEvent *event)
{
...
/*!
  \reimp
*/
void QJulianCalendarWidget::resizeEvent(QResizeEvent * event)
{
    Q_D(QJulianCalendarWidget);
...
/*!
  \reimp
*/
void QJulianCalendarWidget::keyPressEvent(QKeyEvent * event)
{
    Q_D(QJulianCalendarWidget);
...
void QJulianCalendarWidget::setSelectedDateNeedConvert(const QDate &date)
{
    QDate ReturnDate;
    ReturnDate = QDate::fromJulianDay(date.toJulianDay()-13);
...
#include "qjuliancalendarwidget.moc"
#include "moc_qjuliancalendarwidget.cpp"
 
#endif //QT_NO_JULIANCALENDARWIDGET
